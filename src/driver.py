...
class Driver:
    # Create driver in order to load dataset
    def __init__(self):
    ...
    def __load_data():
        def load_file(path):
            data = pa.read_csv(filepath_or_buffer=path, header=None, sep='\s+')
            return data.values
        ...
        def generate_data_path(base_address, group):
            ...
            filepath = os.path.join(base_address, group, 'Inertial Signals')
            for name in Data.FEATURE_TYPE:
                for dimension in Data.DIMENSION:
                    # filenames += [filepath + name + dimension + group + '.txt']
                     filenames.append(os.path.join(filepath, f'{name}{dimension}{group}.txt'))
        ...
        def combine_features(base_address, group):
            ...
            for name in filenames:
                print(f"Attempting to load file: {name}")
                data = load_file(name)
